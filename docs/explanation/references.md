# References

## Books

### Accelerate

 <img src="../assets/references/accelerate.png" width="150" alt="map">

> Forsgren, Nicole and Humble, Jez and Kim, Gene (2018).

### The DevOps Handbook

 <img src="../assets/references/devopshandbook.png" width="150" alt="map">

> Gene Kim, Jez Humble, Patrick Debois, John Willis, Nicole Forsgren (2021).

### Continuous Delivery

 <img src="../assets/references/cdbook.png" width="150" alt="map">

> Jez Humble, Dave Farley.

### Flow Engineering - From Value Stream Mapping to Effective Actions

 <img src="../assets/references/flow-engineering.png" width="150" alt="map">

> Steve Pereira and Andrew Davis (2024)

## PDFs

### State of devops

 <img src="../assets/references/dora-state-of-devops-generic.png" width="150" alt="map">

- [State of AI-assisted Software Development 2025](../assets/lfs/pdf/2025_state_of_ai_assisted_software_development.pdf)

### Trunk based correlated practices

 <img src="../assets/references/correlated-practices.png" width="150" alt="map">

- [Trunk Correlated Practices](assets/lfs/pdf/Trunk_Correlated_Practices_v2.8.pdf)
