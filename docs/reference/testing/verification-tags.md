...
```gherkin
# Feature ID: cli_deployment
# Acceptance Spec: acceptance.spec
# Module: CLI

@cli @critical
Feature: CLI Installation and Deployment
  ...
  @success @ac2 @IV
  Scenario: Verify environment configuration
    Given the CLI is installed
    When I check the environment configuration
    Then all required environment variables should be set
    And the configuration directory should exist

  @success @ac3
  Scenario: Deploy application to staging
    Given the CLI is installed
  ...
  @error @ac4
  Scenario: Handle deployment to invalid environment
    Given the CLI is installed
    When I run "cc deploy invalid-env"
    Then the command should fail
    And stderr should contain "Unknown environment"

  @success @ac5 @PV
  Scenario: Deployment completes within time threshold
    Given the CLI is installed
    And the application is built
    When I run "cc deploy staging"
    Then the deployment should complete within 30 seconds
    And resource usage should stay within limits
```

**Report grouping**:

- **IV (Installation Verification)**: 2 scenarios (ac1, ac2)
- **OV (Operational Verification)**: 2 scenarios (ac3, ac4)
- **PV (Performance Verification)**: 1 scenario (ac5)
...
- [Godog Commands](./godog-commands.md) - Running BDD tests
