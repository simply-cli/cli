...
Feature: src-commands_command-routing
  ...
  Rule: Commands must register themselves via init() and be discoverable
  ...
  Rule: Nested commands must match longest path first
  ...
  Rule: Parent commands without implementation must show available subcommands
  ...
  Rule: Unknown commands must show helpful error messages
  ...
  Rule: Commands must return appropriate exit codes
